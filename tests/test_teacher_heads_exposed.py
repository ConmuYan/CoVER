"""T1 — Teacher multi-head exposure tests for G-OPD-Flash.

Verifies that ``CoVERRelReasoner.forward(..., return_heads=True)`` exposes
the three supervision heads consumed by ``models/flash_adapter.py``:
...
to the teacher's internal pre-tanh sum, so G-OPD-Flash student matching
against them is matching the *right* internal state.
"""
...
def _make_teacher(num_relations: int = 3, hidden_dim: int = 16) -> CoVERRelReasoner:
...
def test_return_heads_false_is_backward_compatible():
    ...
    expected_keys = {
        ...
        "delta_llm", "alpha_llm", "judge_used_mask",
        "fused_rel_h", "relation_strength",
    }
    assert set(out_default.keys()) == expected_keys
    assert set(out_explicit.keys()) == expected_keys

    # Bitwise equality for the legacy keys.
    for k in expected_keys:
        assert torch.equal(out_default[k], out_explicit[k]), f"backward-compat drift on {k!r}"


def test_return_heads_true_exposes_three_heads():
