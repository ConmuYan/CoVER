"""T-CONTRACTS — G-OPD-Flash four hard contracts test suite.

Verifies that ``models/flash_adapter.py::FlashAdapter`` + the G-OPD-Flash
...
* **C2 score-blind input** — static AST pass on ``FlashAdapter.forward``
  rejects any expression that reads ``base_logit`` inside the trunk before
  it is added on as ``s_S = base_logit + Δ_φ``.
* **C3 train-only prototype** — teacher prototype tensors (when relevant)
  are constructed from labelled train indices only; G-OPD-Flash never
  modifies teacher.
...
def test_C2_score_blind_runtime_counterfactual():
    ...
    must NOT change the trunk outputs (``delta_phi``, ``gate_probs``,
    ``gate_logits``, ``c_per_r``, ``s_per_r``).  Only ``final_logit`` may
    change — by exactly the perturbation amount.
    ...
    for p in student.c_per_r_head.parameters():
        torch.nn.init.normal_(p, mean=0.0, std=0.5)
    for p in student.head_gate.parameters():
        torch.nn.init.normal_(p, mean=0.0, std=0.5)
    ...
    trunk_keys = ["delta_phi", "gate_logits", "gate_probs", "c_per_r", "s_per_r", "gate"]
    for k in trunk_keys:
...
def test_C2_dropout_does_not_break_counterfactual():
    ...
    for k in ["delta_phi", "gate_logits", "gate_probs", "c_per_r", "s_per_r"]:
...
def test_C4_delta_bounded_under_extreme_weights(delta_max):
    ...
    base_z, base_logit, rel_features = _make_inputs(base_z_dim=32)
    out = student(base_z, base_logit, rel_features)
    abs_max = float(out["delta_phi"].abs().max())
...
def test_P3_zero_init_all_heads_produce_uniform_gate_at_epoch_0():
    """With zero-init head_gate, gate is uniform 1/R at epoch 0 — student
    has no relation preference until learned."""
    R = 3
    student = FlashAdapter(base_z_dim=32, num_relations=R, hidden_dim=32)
    base_z, base_logit, rel_features = _make_inputs(base_z_dim=32, num_relations=R)
    out = student(base_z, base_logit, rel_features, return_heads=True)
    expected_uniform = torch.full_like(out["gate"], 1.0 / R)
    torch.testing.assert_close(out["gate"], expected_uniform, rtol=1e-5, atol=1e-6)


def test_P3_zero_init_c_per_r_is_zero_at_epoch_0():
    """Zero-init c_per_r_head ⇒ s_per_r = 0 ⇒ c_per_r = gate · 0 = 0."""
    student = FlashAdapter(base_z_dim=32, num_relations=3, hidden_dim=32)
    base_z, base_logit, rel_features = _make_inputs(base_z_dim=32)
    out = student(base_z, base_logit, rel_features, return_heads=True)
    assert out["s_per_r"].abs().max() < 1e-6
    assert out["c_per_r"].abs().max() < 1e-6
...
def test_back_compat_alias_is_FlashAdapter():
    from models.rel_distill_adapter import RelDistillAdapter as RDA_shim
    from models.flash_adapter import RelDistillAdapter as RDA_native, FlashAdapter
    assert RDA_shim is RDA_native is FlashAdapter, (
        "back-compat alias drift between models.rel_distill_adapter and models.flash_adapter"
    )
