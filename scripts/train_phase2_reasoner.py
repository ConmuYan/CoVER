...
def run_training(
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_interval == 0 or epoch == epochs:
            val_metrics, _, _ = evaluate_split(
                reasoner, base_z, base_logits, rel_features, val_mask, y, device,
            )
            diag = compute_full_diagnostics(reasoner, base_z, base_logits, rel_features, device)
    ...
    test_metrics, _, _ = evaluate_split(
        reasoner, base_z, base_logits, rel_features, test_mask, y, device,
        threshold=threshold,
    )
    diag_test = compute_full_diagnostics(reasoner, base_z, base_logits, rel_features, device)
