"""Aggregate Phase2 reasoner experiments and write CSV/MD/markdown report.

Inputs (per experiment, per seed):
  artifacts/results/{ds}/bwgnn/phase2_E{i}_{name}/seed_{seed}/stage3_metrics.json
  artifacts/logs/{ds}/bwgnn/phase2_E{i}_{name}/seed_{seed}/phase2_diagnostics.json

Outputs:
  artifacts/tables/phase2_5seed_summary.csv
  artifacts/tables/phase2_5seed_summary.md
  artifacts/reports/phase2_first_round_conclusion.md
"""
...
EXPERIMENTS = [
    ...
    ("E3", "phase2_E3_no_trust"),
]
DEFAULT_SEEDS = [42, 123, 456, 789, 2026]

# Reference baselines (from PROGRESS.md / 5-seed reports already on disk).
BASELINES = {
...
def _load_json(p: Path) -> dict | None:
    ...
    try:
        with p.open() as f:
            return json.load(f)
    except json.JSONDecodeError as exc:  # surface corrupted artifacts
...
def collect(seeds: list[int]) -> tuple[list[dict], list[dict]]:
    ...
    for ds in DATASETS:
        for exp_id, run_name in EXPERIMENTS:
            ...
            for seed in seeds:
                metrics_path = PROJECT_ROOT / "artifacts" / "results" / ds / "bwgnn" / run_name / f"seed_{seed}" / "stage3_metrics.json"
                diag_path = PROJECT_ROOT / "artifacts" / "logs" / ds / "bwgnn" / run_name / f"seed_{seed}" / "phase2_diagnostics.json"
            ...
            agg = {
                ...
                "experiment": exp_id,
                "run_name": run_name,
                "n_seeds_present": sum(1 for s in seeds if (PROJECT_ROOT / "artifacts" / "results" / ds / "bwgnn" / run_name / f"seed_{s}" / "stage3_metrics.json").exists()),
            }
            for m in METRICS:
...
def write_md_summary(summary: list[dict], path: Path) -> None:
    lines: list[str] = ["# Phase2 5-seed Summary", ""]
    for ds in DATASETS:
        lines.append(f"## {ds}")
        ...
        for label in ("BWGNN base", "anchor_gate", "judge_strength_gate"):
            row = BASELINES.get((ds, label), {})
            lines.append(
                f"- **{label}**: AUPRC {fmt(row.get('auprc'))} | ROC-AUC {fmt(row.get('roc_auc'))} | Macro-F1 {fmt(row.get('macro_f1'))} | G-Means {fmt(row.get('g_means'))}"
            )
        lines.append("")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n")
...
def write_conclusion_report(summary: list[dict], path: Path) -> None:
    """Five-section first-round conclusion."""
    ...
    lines: list[str] = []
    lines.append("# Phase2 First-Round Conclusion")
    lines.append("")
    lines.append("## 1. E0 reproduction check vs existing anchor_gate")
    lines.append("")
    lines.append("| Dataset | E0 AUPRC (new reasoner) | anchor_gate baseline | Δ |")
    lines.append("|---|---|---|---|")
    for ds in DATASETS:
        new = lookup_metric(ds, "E0", "auprc")
        base = BASELINES[(ds, "anchor_gate")]["auprc"]
        delta = new - base if isinstance(new, (int, float)) and not math.isnan(new) else float("nan")
        lines.append(f"| {ds} | {fmt(new)} | {fmt(base)} | {fmt(delta)} |")
    lines.append("")
    lines.append("Pass criterion: |Δ| ≤ 0.005. If Δ < -0.005 the new reasoner regresses; investigate softmax-gate / loss differences.")
    lines.append("")
    ...
    lines.append("## 3. Comparison with prior baselines")
    lines.append("")
    for ds in DATASETS:
        lines.append(f"### {ds}")
        lines.append("")
        ...
        lines.append("|---|---|---|---|---|")
        for label in ("BWGNN base", "anchor_gate", "judge_strength_gate"):
            row = BASELINES[(ds, label)]
            lines.append(f"| {label} | {fmt(row['auprc'])} | {fmt(row['roc_auc'])} | {fmt(row['macro_f1'])} | {fmt(row['g_means'])} |")
        for exp_id, _ in EXPERIMENTS:
            ent = lookup.get((ds, exp_id), {})
            lines.append(
                f"| Phase2 {exp_id} | {fmt(ent.get('auprc_mean'))} | "
    ...
    lines.append("")
    lines.append("- **E0 reproduction**: see Section 1.")
    lines.append("- **E1 (judge alignment only)**: compare to E0 — does L_align improve gate alignment without changing the logit?")
    lines.append("- **E2 (judge residual)**: compare to E1 — does small α residual buy further AUPRC?")
    lines.append("- **E3 (no trust)**: compare to E2 — does removing L_trust cause regression / instability (look at mean|Δ_rel| inflation)?")
    lines.append("- **YelpChi vs Amazon**: confirm RUR-dominant vs UVU-distributed gate behavior.")
    lines.append("")
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    write_csv(per_seed, PROJECT_ROOT / "artifacts" / "tables" / "phase2_5seed_per_seed.csv")
    write_csv(summary, PROJECT_ROOT / "artifacts" / "tables" / "phase2_5seed_summary.csv")
    write_md_summary(summary, PROJECT_ROOT / "artifacts" / "tables" / "phase2_5seed_summary.md")
    write_conclusion_report(summary, PROJECT_ROOT / "artifacts" / "reports" / "phase2_first_round_conclusion.md")
    print(f"Aggregated {len(per_seed)} per-seed rows into 8 (ds × exp) summary entries.")
    return 0
