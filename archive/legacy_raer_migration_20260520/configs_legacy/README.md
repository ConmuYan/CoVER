...
two-stage **RAER/LREE/CBR** framework with a cls-only teacher and a
contract-budgeted residual student.

```
Phase 1 : train a fresh base detector (BWGNN / GraphSAGE / GCN / GAT)
          → frozen as structural prior (SHA-256 verified)
Phase 2 : train a RAER teacher over relation-aware
...
└── phase2_reasoner/
    └── ablation/                                      # RAER-HC ablation suite (7 cells)
        ├── idea1_canonical_clsonly.yaml               # RAER-HC baseline (★)
...
| `phase2_reasoner/ablation/idea1_canonical_clsonly.yaml` | RAER-HC baseline. Anchors all paired-t comparisons. |
| `phase2_reasoner/ablation/idea1_ablate_*.yaml` | RAER-HC ablation cells; only one structural toggle changes per file. |

## RAER-HC ablation toggles (only knobs that vary)
...
- **New Phase 2 ablation cell** → `configs/phase2_reasoner/ablation/<name>.yaml`,
  ...
  introduce a new toggle only after wiring it through `models/raer_teacher.py`.
