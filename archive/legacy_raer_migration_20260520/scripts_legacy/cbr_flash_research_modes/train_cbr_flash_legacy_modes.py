"""CBR-Flash trainer — contract-budgeted residual distillation.

Implements the lightweight student stage of the final RAER/LREE/CBR story.
The historical mode names are preserved to keep existing result directories
and aggregators readable, but the canonical script name is
...
* ``g_opd_flash``       — historical name for the full CBR-Flash sampler.
...
Per-epoch training loop::
    ...
    # Phase D — entropy-aware mixed Bernoulli KL with CBR weighting
...
Outputs::
    ...
    artifacts/results/{ds}/{base}/g_opd_flash_{mode}/seed_{s}/test_metrics.json
    artifacts/results/{ds}/{base}/g_opd_flash_{mode}/seed_{s}/raer_summary.json
...
from evidence.lree import build_learned_extractor
...
# ═══════════════════════════════════════════════════════════════════════════
# Teacher cache with 3 heads (for CBR-Flash modes)
# ═══════════════════════════════════════════════════════════════════════════
    ...
    for epoch in range(1, epochs + 1):
        ...
        if mode == "off_policy":
            ...
            loss, stats, _ = _compute_distill_terms_on_indices(
                ...
                # interp-A passthrough: allow purifying det_mask_cbr (drop
                # reliability / adaptive λ_bce / mixed-KL) so the 4-condition
                # incremental ablation matrix (BCE / +rKL / +CBR / +rKL+CBR)
                # tests the genuinely locked 3-component loss form.
                ablate_reliability=bool(distill_cfg.get("ablate_reliability", False)),
                ablate_adaptive_bce=bool(distill_cfg.get("ablate_adaptive_bce", False)),
                ablate_mixed_kl=bool(distill_cfg.get("ablate_mixed_kl", False)),
            )
        ...
        if epoch % 5 == 0 or epoch == epochs:
            ...
            if mv > best_val:
                ...
                if no_improve >= patience:
                    print(f"[CBR-Flash:{mode}] Early stop at epoch {epoch} (best val_auprc={best_val:.4f} @ {best_epoch})")
                    break
...
train_g_opd_flash = train_cbr_flash
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="CBR-Flash trainer")
    ...
    p.add_argument("--teacher_extractor_ckpt", type=str, default=None,
                   help="Optional LREE lree.pt for the teacher")
    ...
    # Distillation hyperparams (CBR-Flash defaults)
    ...
    # ── interp-A purify flags (incremental 4-condition loss ablation) ──
    # Applied to the det_mask_cbr branch only.  Default False = canonical
    # behaviour; set all three True to obtain the genuinely locked
    # 3-component loss form L_bce^(K) + α_f·rKL^(K) + λ_cbr·L_CBR^(K).
    p.add_argument("--ablate_reliability", action="store_true",
                   help="interp-A: r_node ≡ 1 (drop reliability weighting)")
    p.add_argument("--ablate_adaptive_bce", action="store_true",
                   help="interp-A: λ_bce_i ≡ λ_min (drop adaptive BCE anchor)")
    p.add_argument("--ablate_mixed_kl", action="store_true",
                   help="interp-A: η ≡ 0 (pure reverse-KL only)")
...
def main() -> None:
    ...
    print(f"[CBR-Flash:{mode}] Base SHA-256 (epoch 0) = {base_sha256[:16]}...")
    ...
    relation_names: list[str] = [
        ...
        raer_cfg.get("relation_names", rel_meta.get("relations", []))
    ]
    ...
    teacher.eval()
    print(f"[CBR-Flash:{mode}] Loaded teacher from {teacher_ckpt_path}")
    ...
    print(f"[CBR-Flash:{mode}] Teacher cache (3 heads) generated in {time.time() - t0:.1f}s")
    ...
    print(f"[CBR-Flash:{mode}] cell_key={cell_key} r_c={r_c:.3f} cal_bins={'loaded' if cal_bins else 'fallback=1.0'}")
    ...
    print(f"[CBR-Flash:{mode}] Student params: {n_student} ({n_student / n_teacher:.2f}x teacher)")
    ...
    distill_cfg = {
        ...
        # interp-A ablation passthrough (det_mask_cbr branch only)
        "ablate_reliability": args.ablate_reliability,
        "ablate_adaptive_bce": args.ablate_adaptive_bce,
        "ablate_mixed_kl": args.ablate_mixed_kl,
    }
    t0 = time.time()
    ...
    print(f"[CBR-Flash:{mode}] C1 contract OK: base SHA-256 unchanged (post-training)")

    # ----- T7: deployment-shift eval (optional) -----
    deploy_shift_metrics = None
    if args.base_v2_ckpt_path is not None:
        print(f"[CBR-Flash:{mode}] T7 deployment-shift eval: loading base_v2 from {args.base_v2_ckpt_path}")
    ...
    tt = teacher_metrics
    print(f"\n[CBR-Flash:{mode}] Done in {elapsed:.1f}s | best_epoch={train_summary['best_epoch']}")
