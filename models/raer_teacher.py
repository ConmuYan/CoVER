"""RAER teacher: relation-aware evidence reasoning with a bounded residual.

Forward (rel-only)::

    z_i = b_i + Δ_rel_i,   |Δ_rel_i| ≤ delta_rel_max

This is the canonical teacher for the two-stage RAER distillation framework.
Only score-blind relation evidence is used. All regularisation is structural:
bounded ``tanh`` saturation, zero-initialised per-relation residual heads,
``detach``-isolated base inputs, and independent per-relation expert MLPs.
...
class RAERTeacher(nn.Module):
    """RAER teacher with a softmax relation gate.
    ...
    def __init__(
        ...
        # ----- RAER-HC ablation toggles (config-driven switches) -----
    ...
    def forward(
        ...
        Args:
            ...
            return_heads: If True, the return dict additionally exposes
                diagnostic quantities for the teacher's internal relation
                decomposition:
        ...
        # Gate-weighted fused expert hidden for diagnostics and downstream
        # CBR-Flash supervision.
        ...
        if return_heads:
            # Teacher diagnostics: expose the exact internal relation
            # decomposition without making these student matching targets.
